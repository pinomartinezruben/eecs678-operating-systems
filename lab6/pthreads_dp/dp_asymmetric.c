...
static void *dp_thread(void *arg)
{
  ...
  while (!Stop) {
    ...
    // condition if the philosopher is even
    if (me->id % 2 == 0) {
      // since philosopher is even number, they eat from left to right
      pthread_mutex_lock(left_chop(me));
      pthread_mutex_lock(right_chop(me));
    }
    else {
      // since philosopher is odd number, they eat from right to left
      pthread_mutex_lock(right_chop(me));
      pthread_mutex_lock(left_chop(me));
    }
