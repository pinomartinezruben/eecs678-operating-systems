...
int main(int argc, char *argv[])
{
  ...
  if (pid_1 == 0) {
    ...
    char cmdbuf[BSIZE];
    bzero(cmdbuf, BSIZE);
    sprintf(cmdbuf, "%s %s -name \'*\'.[h]", FIND_EXEC, argv[1]);
  ...
  if (pid_2 == 0) {
    ...
    if ((execl(XARGS_EXEC, XARGS_EXEC, GREP_EXEC, "-c", argv[2], (char *)NULL)) < 0) {
  ...
  if (pid_3 == 0) {
    ...
    if ((execl(SORT_EXEC, SORT_EXEC, "-t", ":", "+1.0", "-2.0",
               "--numeric", "--reverse", (char *)NULL)) < 0) {
  ...
  if (pid_4 == 0) {
    ...
    close(p3[1]);
    //STEP 8
    //Invoke execl for head (use HEAD_EXEC as path). Print only the first 6 results in the output
    char headbuf[BSIZE];
    bzero(headbuf, BSIZE);
    sprintf(headbuf, "--lines=%s", argv[3]);
    if ((execl(HEAD_EXEC, HEAD_EXEC, headbuf, (char *)NULL)) < 0) {
